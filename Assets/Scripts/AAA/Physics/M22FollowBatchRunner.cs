...
{
    public sealed class M22FollowBatchRunner : MonoBehaviour
    {
        ...
        [SerializeField] private float followEnglish = 0.75f;
        ...
        [SerializeField] private string outputFileName = "follow_runtime_measurements.json";
        ...
        [Serializable] private sealed class Sample
        {
            ...
            public float followDot;
            public bool followPass;
        }

        [Serializable] private sealed class Output
        {
            public string timestampUtc;
            public string caseType = "Follow";
            public string scene;
        ...
        private void Start()
        {
            ...
            {
                Debug.LogError("[147VR M2.2] Missing physics setup, balls, or CuePhysicsAdapter.", this);
                enabled = false;
                return;
            }
        ...
        private bool NormalizeBodies()
        {
            ...
            Debug.Log($"[147VR M2.2] SURFACE topY={y:F6} cueY={cueBall.position.y:F6} objectY={objectBall.position.y:F6} followEnglish={followEnglish:F3}");
            return true;
        }

        private void InstallProbe(Rigidbody body)
        {
            var probe = body.GetComponent<M22FollowCollisionProbe>();
            if (probe == null) probe = body.gameObject.AddComponent<M22FollowCollisionProbe>();
        ...
        private void BeginShot()
        {
            ...
            if (!physicsAdapter.Apply(shot, new Vector2(0f, followEnglish)))
            {
                Debug.LogError("[147VR M2.2] Follow impulse rejected.", this);
                enabled = false;
                return;
            }
            Debug.Log($"[147VR M2.2] REAL FOLLOW shot {completedShots + 1}/{repetitions} speed={shotSpeed:F3} englishY={followEnglish:F3}");
        }
        ...
        private void FixedUpdate()
        {
            ...
            {
                Debug.LogError($"[147VR M2.2] TIMEOUT rep={completedShots + 1}/{repetitions} collision={targetCollisionObserved} cue={cueBall.position} object={objectBall.position}");
                enabled = false;
            }
        }
        ...
        internal void NotifyTargetCollision(Collision collision)
        {
            ...
            Debug.Log($"[147VR M2.2] Tcollision | other={collision.collider.name} contact={collision.GetContact(0).point} normal={collision.GetContact(0).normal} cueV={cueVelocityAtContact} objectV={objectVelocityAtContact}");
        ...
        private void CompleteShot()
        {
            ...
            float followDot = postSpeed > 0.000001f ? Vector3.Dot(cueVelocityAfterContact.normalized, shotDirection) : 0f;
            float firstFlight = Vector3.Distance(objectStart, objectPositionAtContact);
            bool pass = targetCollisionObserved && postSpeed >= 0.05f && followDot >= 0.5f;
            ...
            completedShots++;
            Debug.Log($"[147VR M2.2] rep={completedShots}/{repetitions} postCueSpeed={postSpeed:F6} cuePostDistance={cuePostDistance:F6} firstFlight={firstFlight:F6} cueResidual={cueResidual:F6} followDot={followDot:F6} objectPeak={objectPeak:F6} {(pass ? "PASS" : "FAIL")}");
            if (completedShots >= repetitions)
            {
                Persist();
                Debug.Log("[147VR M2.2] REAL FOLLOW MEASUREMENT COMPLETE");
                enabled = false;
                return;
            }
        ...
        private void Persist()
        {
            ...
            var output = new Output
            {
                ...
                scene = "Assets/AAA/PhysicsCalibration/147VR_M22_FollowCalibration.unity",
                unityVersion = Application.unityVersion,
                repetitions = samples.Count,
                shotSpeed = shotSpeed,
                followEnglish = followEnglish,
                ...
                followDot = ToArray(s => s.followDot),
                passed = ToBoolArray(s => s.followPass)
            };
            string path = Path.Combine(dir, outputFileName);
            File.WriteAllText(path, JsonUtility.ToJson(output, true));
            Debug.Log($"[147VR M2.2] Persisted REAL Follow JSON: {path}");
        }
    ...
    public sealed class M22FollowCollisionProbe : MonoBehaviour
    {
        private M22FollowBatchRunner runner;
        private Rigidbody target;

        public void Initialize(M22FollowBatchRunner owner, Rigidbody targetBody)
        {
            runner = owner;
            target = targetBody;
        }
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            Debug.Log($"[147VR M2.2] COLLISION | body={name} other={collision.collider.name} point={contact.point} normal={contact.normal} relV={collision.relativeVelocity.magnitude:F6}");
