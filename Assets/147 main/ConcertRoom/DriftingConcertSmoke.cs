...
public class DriftingConcertSmoke : MonoBehaviour
{
    ...
    void Configure()
    {
        ...
        if (psRenderer != null)
        {
            ...
            // Use the URP particle shader first; the legacy shader may not exist
            // in an Android/URP build. Never construct a Material from a null shader.
            Shader particleShader = Shader.Find("Universal Render Pipeline/Particles/Unlit");
            if (particleShader == null)
                particleShader = Shader.Find("Particles/Standard Unlit");
            if (particleShader == null)
            {
                Debug.LogError("[DriftingConcertSmoke] No supported particle shader found; smoke disabled.");
