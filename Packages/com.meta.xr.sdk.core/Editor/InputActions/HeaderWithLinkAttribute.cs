...
public class InlineLinkDrawer : DecoratorDrawer
{
    ...
    public override void OnGUI(Rect position)
    {
        ...
        position.x -= EditorGUIUtility.singleLineHeight * 1.0f;
        // Magic number adjustment.
        position.y -= 2f;

        if (EditorGUI.LinkButton(position, "[?]"))
        {
            Application.OpenURL(Attribute.DocumentationURL);
        }
