...
{
    /// <summary>
    /// Service for OpenCode CLI integration in Unity Editor.
    /// Uses a one-shot process-per-prompt model.
    /// </summary>
    [RegisterAIService(ServiceId, "OpenCode", Priority = 40, ExecutableName = "opencode", SkillsSubPath = ".opencode/skills")]
    public class OpenCodeService : AIServiceBase, IServiceSettingsUI, IServiceValidation
    {
            ...
            string? systemPrompt = null)
        {
            ...
            try
            {
                ...
                await ExecuteOpenCodeAsync(userInput, systemPrompt, caller);
        ...
        private async Task ExecuteOpenCodeAsync(string userInput, string? systemPrompt, CallerIdentity? caller)
        {
            if (systemPrompt != null)
                Log.Warning("OpenCode CLI does not support system prompts — the systemPrompt parameter will be ignored.");

            var executable = ResolveExecutable(OpenCodeSettings.ExecutablePath.Value, "opencode");
            Log.Info($"Executing OpenCode (caller: {caller?.Id ?? "default"})");
            ...
            var readTask = Task.Run(() =>
            {
                ...
                {
                    Log.Warning($"Error reading OpenCode output: {ex.Message}");
                }
            });
            ...
            if (!completed)
            {
                ...
                throw new TimeoutException("OpenCode CLI timed out after 10 minutes");
            }
            ...
            else if (process.ExitCode != 0)
            {
                ...
                throw new Exception($"OpenCode exited with code {process.ExitCode}: {stderr}");
            }
        }
        ...
        public override async Task CancelCurrentOperationAsync()
        {
            ...
            if (processToKill != null)
            {
                ...
                {
                    Log.Warning($"Error killing OpenCode process: {ex.Message}");
                }
            }
        }
        ...
        {
            UnityEditor.EditorGUILayout.LabelField("OpenCode Settings", UnityEditor.EditorStyles.boldLabel);
            OpenCodeSettings.ExecutablePath.DrawForGUI(origins, originData);
        }
        ...
        public async Task<ValidationResult> ValidateConfigurationAsync()
        {
            ...
            try
            {
                ...
                var processStartInfo = new ProcessStartInfo
                {
                    FileName = ResolveExecutable(configuredPath, "opencode"),
                    UseShellExecute = false,
                    RedirectStandardOutput = true,
                    RedirectStandardError = true,
                    CreateNoWindow = true
                ...
                {
                    _currentValidationResult = ValidationResult.Invalid("OpenCode CLI not found or not configured");
                }
            }
